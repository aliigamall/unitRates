...
export default function ItemComponent(vido, props) {
  const {
    api,
    state,
    onDestroy,
  ...
  onDestroy(
    state.subscribe(
      "config.wrappers.ChartTimelineItemsRowItem",
      value => (wrapper = value)
    )
  );
  let itemLeftPx = 0,
  ...
  function updateItem() {
    ...
    itemWidthPx =
      (props.item.time.end - props.item.time.start) / time.timePerPixel;
    ...
    if (props.item.time.start < time.leftGlobal) {
      leftCutStyleMap.style["margin-left"] =
        (time.leftGlobal - props.item.time.start) / time.timePerPixel + "px";
      cutLeft = true;
    } else {
    ...
    if (props.item.time.end > time.rightGlobal) {
      rightCutStyleMap.style["margin-right"] =
        (props.item.time.end - time.rightGlobal) / time.timePerPixel + "px";
      cutRight = true;
    } else {
      cutRight = false;
    ...
    styleMap.setStyle({});
    const inViewPort = api.isItemInViewport(
      props.item,
      time.leftGlobal,
      time.rightGlobal
    );
    ...
    for (const parentId of props.row._internal.parents) {
      ...
      if (childrenStyle)
        styleMap.setStyle({ ...styleMap.style, ...childrenStyle });
    }

    if (props.row.style) {
      const currentRowItemsStyle = props.row.style.items.item.current;
      if (currentRowItemsStyle)
        styleMap.setStyle({ ...styleMap.style, ...currentRowItemsStyle });
    }
  ...
  function onPropsChange(changedProps, options) {
    if (
      options.leave ||
      changedProps.row === undefined ||
      changedProps.item === undefined
    ) {
      leave = true;
      shouldDetach = true;
      return update();
  ...
  onDestroy(
    state.subscribeAll(
      ["_internal.chart.time", "config.scroll.compensation.x"],
      updateItem
    )
  );
